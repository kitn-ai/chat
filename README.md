...
It asks for a framework and a backend, then writes a project that runs. Anything with one possible answer today — the layout, the feature set — it states instead of asking. Press Enter through every prompt and you get React, full screen, and the kit's own mock: a real streaming turn on the first `npm run dev`, with no API key and nothing to host.
