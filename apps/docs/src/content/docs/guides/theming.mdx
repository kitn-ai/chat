...
`theme.tokens.css` skips the first row and the `@utility` row. It still declares the same `--color-*`, `--radius-*` and `--text-*` names on `:root`, and Tailwind v4 utilities read those variables at runtime, so the radius values above change under it as well. If you want none of the kit's scale, import neither file and theme through `--kai-*`.
