...
Pin an exact version in production (e.g. `@kitn.ai/ui@0.25.0/dist/...`). The package is pre-1.0, so a minor release can break things. Pin `0.25.0` or newer: `0.14.1` through `0.24.0` are covered by a [critical security advisory](https://github.com/kitn-ai/ui/security/advisories), and a CDN fetch prints no deprecation warning to tell you.
</Aside>
