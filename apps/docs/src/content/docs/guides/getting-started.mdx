...
Pin an exact version in production (e.g. `@kitn.ai/ui@0.30.0/dist/...`).
...
This bundle registers everything in one fetch — right for a chat page. A page placing only an element or two can load the [autoloader](/guides/loading/#autoloader) instead, which fetches each element on demand.
</Aside>
