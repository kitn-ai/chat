...
<Aside type="note" title="What the `.` entry resolves to">
The package ships **compiled**. `@kitn.ai/ui` resolves to `dist/index.js` in the browser and `dist/index.server.js` under the `node` / `deno` / `worker` conditions. Both are ESM, so your bundler still tree-shakes down to what you import, but it never compiles the kit's TSX. A stock `vite-plugin-solid()` is enough; nothing has to be told to transform `node_modules`.
</Aside>
