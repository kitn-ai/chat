...
By default each element is styled inside its own Shadow DOM and needs no external CSS. To override design tokens (colors, radii, spacing), load `theme.tokens.css` — the compiled plain-CSS build. A raw `<link>` can't use `theme.css` itself: that file is Tailwind v4 *source* (its tokens live in an `@theme` block a plain stylesheet load discards whole), so it only works when a bundler runs Tailwind over it.
