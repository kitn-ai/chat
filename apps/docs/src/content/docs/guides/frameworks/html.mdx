...
`kai.es.js` registers every element in one fetch — the right shape for a `<kai-chat>` page, which pulls most of the kit anyway. A page placing only an element or two does better with the [autoloader](/guides/loading/#autoloader), which fetches each element on demand. The streaming helpers work here too: `dist/state.js` and `dist/wire.js` are self-contained ES modules, so `@kitn.ai/ui/state` and `@kitn.ai/ui/wire` load from the same CDN URLs with no bundler.
</Aside>
...
Each element styles itself inside its own Shadow DOM and needs no external CSS. To retheme the elements, set **`--kai-color-*`** custom properties on `:root` (or a scoping selector) — inherited custom properties cross the shadow boundary, and every element resolves its colors through the `--kai-` names:
...
The unprefixed `--color-*` names do **not** work here: each shadow root declares them on `:host`, which beats anything inherited from your `:root`. No stylesheet link is needed either — the elements carry their own CSS. Load `theme.tokens.css` only when you want the kit's `--color-*` tokens for your **own** page chrome around the elements:
