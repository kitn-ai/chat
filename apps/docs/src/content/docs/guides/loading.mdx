...
| **Autoloader** | `<script>` (CDN / static) | No build step and a handful of elements. Loads each element on demand. |
...
An unversioned CDN URL tracks the latest release, which is risky on a pre-1.0 package. Pin an exact version: `https://cdn.jsdelivr.net/npm/@kitn.ai/ui@0.31.0/dist/elements/autoloader.js`.
