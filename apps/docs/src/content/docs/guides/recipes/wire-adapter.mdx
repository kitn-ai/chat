...
The kit never calls your function. `readOpenAIStream` returns the turn, you run the tools, and `applyToolOutput` completes the panel:
...
for (const call of turn.toolCalls) {
  if (call.error || call.providerExecuted) continue;
  const output = await runYourTool(call.name, call.input ?? {});
  applyToolOutput(stream, call.id, output);
}
// then POST again with toOpenAIMessages(latestMessages) to let the
// model answer with the results. Cap the rounds: a runaway model is
// a runaway bill.
```
