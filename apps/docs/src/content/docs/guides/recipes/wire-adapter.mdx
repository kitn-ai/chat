...
The kit never calls your function. `readOpenAIStream` returns the turn, you run the tools, and `applyToolOutput` completes the panel. Every round folds onto the same assistant message, so one stream spans the whole loop:
...
try {
  ...
  for (let round = 0; round < 4; round++) {
    const res = await fetch('/api/chat', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      // Re-encoded every round, so the results applyToolOutput just wrote travel
      // back with the next request. That is what lets the model answer with them.
      body: JSON.stringify({ messages: toOpenAIMessages(chat.messages), tools }),
    });
    ...
    for (const call of pending) {
      const output = await runYourTool(call.name, call.input ?? {});
      applyToolOutput(stream, call.id, output);
    }
  }
} finally {
  stream.done();
