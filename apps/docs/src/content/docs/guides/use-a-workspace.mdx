...
<script type="module">
  ...
  // The sidebar list. With no `groups`, every row renders in one Ungrouped section.
  ws.conversations = [
    {
      id: 'c1',
...
`messages` is the same shape `<kai-chat>` uses: `id`, `role`, an ordered `parts` array (`text`, `reasoning`, `tool`, `card`, `source`, `file`), plus optional `actions`.
