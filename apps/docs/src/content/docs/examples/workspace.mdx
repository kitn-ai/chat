...
<p class="kai-lede">Three elements give you the full workspace: the `<kai-workspace>` layout shell, a resizable, collapsible `<kai-conversations>` rail listing past conversations, and a streaming `<kai-chat>` thread. Select a conversation, start a new one, or send a message below.</p>
