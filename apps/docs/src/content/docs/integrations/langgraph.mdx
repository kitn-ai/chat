...
The loop above renders the answer. To show the agent's *work* — the tool calls and reasoning LangGraph emits — map them onto the message contract and forward extra frames a richer browser reader picks up. A `<kai-chat>` message's `parts` array carries `tool` and `reasoning` parts alongside its `text` part, each tool part moving through a `state`:

| LangGraph piece | Message contract field |
|---|---|
| `messageChunk.content` (string) | a `{ type: 'text' }` part's `text` |
| `chunk.tool_call_chunks[].name` / `.args` | a `{ type: 'tool' }` part's `tool.input` (`input-streaming` → `input-available`) |
| `ToolMessage.content` | the tool part's `tool.output` (`output-available`) |
| `additional_kwargs.reasoning_content` | a `{ type: 'reasoning' }` part's `text` |
