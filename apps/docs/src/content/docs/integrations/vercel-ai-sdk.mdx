...
`streamText({ model, messages })` runs the model and returns a result that streams lazily. `<kai-chat>` messages carry ordered `parts`, not a `content` string, so encode the thread with `toOpenAIMessages(history)` (from `@kitn.ai/ui/wire`) before posting — the result is `{ role, content }` pairs, which `streamText` takes directly. The result exposes `textStream`, an async iterable of string deltas, and that's the primitive you re-frame into SSE.
