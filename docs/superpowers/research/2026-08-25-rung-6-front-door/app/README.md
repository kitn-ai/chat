# Compose-your-own-thread (rung 6)
...
third scripted turn emits a `search_docs` tool call. Attach a file with the
paperclip, send, and it rides as a `file` part on your message into the thread.

Rung 6 of the iteration ladder — the clean-room "compose your own thread"
front-door build. Plain Vite + TypeScript, no framework.

Run: `npm install` then `npm run dev` (:5173) · Build: `npm run build`.
