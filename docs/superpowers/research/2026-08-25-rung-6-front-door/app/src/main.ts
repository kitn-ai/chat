...
// Object URLs minted for staged/sent attachments. Revoked when an attachment
// is dropped before sending, and on unload — never while the thread still
// renders it, which would blank the preview.
...
async function main() {
  ...
  fileInput.addEventListener('change', () => {
    ...
    staged = [
      ...staged,
      ...picked.map((file): AttachmentData => {
  ...
  stagedEl.addEventListener('kai-remove', (event) => {
    ...
    // Safe to revoke: a staged attachment has not reached the thread yet.
