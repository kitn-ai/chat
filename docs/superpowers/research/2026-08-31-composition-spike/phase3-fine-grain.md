...
| Fine grain, after both fidelity rounds | 218 | 257 | **475** |

**~2.6×** — and the comparison still flatters the facade's JS: 45 of `app.js`'s 147 lines are the hand-rolled store that phase 2's gap forced, which `fine.js` replaces with one import from `/kit/stores.js`. Like for like (both on `stores.js`), the facade page would be ~141 lines total, putting the fine grain nearer **3.4×**. The first pass's 1.8× was an undercount by exactly the fidelity debt: matching the facade's actual navigation, tab bar, home-screen anatomy, list presentation and palette added ~150 lines across the two rounds, almost all of it chrome markup/CSS and routing glue — F-1/F-2/F-3, precisely the unpackaged product tier.
