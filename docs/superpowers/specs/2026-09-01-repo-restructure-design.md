...
## Rulings after the extraction and CI investigations (2026-09-01, later the same day)

**MCP stays in the ui package, out of `src/`: `packages/ui/mcp/`.** The read-only investigation (`docs/superpowers/research/2026-09-01-mcp-extraction-surface.md`) found the code boundary small (4 unexported values + 2 assets) but the repo boundary large: ~104 references in 55 files point into `agent-tooling`, 36 build/test plumbing sites name it, four generators/guards would become cross-package, `component_reference`'s manifest lookup would need rewriting, and the builder components' import of the construct engine makes a naive cut cyclic. The owner ruled that reorganizing must not make wiring harder, so this round does the Step-1 trick instead: move `src/agent-tooling` to `packages/ui/mcp/` with its own tsconfig (today's `tsconfig.mcp.json` body) and its tests alongside, one PR, the npm package byte-identical. A true `packages/mcp` stays banked in that research doc for when the MCP needs its own publish cadence (its recommended shape then: private workspace package bundled into ui's dist). Shipping an MCP inside the library's npm package is the normal pattern (`npx shadcn mcp`), so nothing consumer-facing was ever at stake. The map table above is amended accordingly: `packages/mcp` → `packages/ui/mcp/` for now.

**Parallel CI split approved as designed** (`docs/superpowers/research/2026-09-01-ci-split-design.md`): five legs (`build` · `construct` · `unit` · `dist-guards` · `browser`) behind a `test` aggregator with `if: always()` and a verdict derived from `toJSON(needs)`; the required-check name `test` is kept; `lint-gate-parity` and the fourteen `jobBlock(yaml,'test')` wiring tests learn that the gate is `test` plus its transitive `needs:`, with a new hard failure for a gate-bearing job unreachable from `test`; the npm cache restore moves ahead of the emit chain; both stale "advisory" comment blocks are rewritten; the artifact glob gains a coverage check; `docs/coupling-map.md` gains two rows. Proven by diffing `lint-gate-parity --list` across base and branch with the new linter on both, and three watched red runs (a failed leg, a skipped leg, a gate moved outside the graph). Sequencing: CI split first, then the MCP move, then blocks, then the pages to `apps/`.
