...
import { buildSystemPrompt, runTool, toolsFor } from '../tools';
    ...
    async (raw: string) => {
      ...
      try {
        for (let round = 0; round < MAX_ROUNDS; round++) {
          const res = await openChatStream({ messages: encode(), tools, cardMode });
          ...
          if (usable.length > 0) {
            for (const call of usable) {
              ...
              // Sources and cards are message PARTS now, so they land inside the
              // assistant turn in stream order instead of in a tray under it.
