...
describe('the committed fixtures', () => {
  ...
  it('are read from live/ only, never from the canned streams', () => {
    ...
    // A glob over `fixtures/` therefore counts ~170 files against 138 usage
    // frames and screams truncation at a set that was never recorded — the same
    // defect class this guard exists for, one directory over.
