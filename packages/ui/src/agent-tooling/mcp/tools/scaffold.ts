...
function renderSolid(archetype: Archetype, ctx: RenderCtx): string {
  ...
  const surface = (pad: string, fill: string): string[] =>
    [
      ...
      `                  <Index each={m().parts}>{(part) => renderPart(part, m().role)}</Index>`,
      `                </Message>`,
      `              )}`,
      `            </Show>`,
      `          )}`,
  ...
  return [
    ...
    `// delta — a panel that stays open but stops updating.`,
    `function renderPart(part: () => MessagePart, role: ChatMessage['role']) {`,
    `  return (`,
    `    // card / source / file parts hit the fallback — see the docs for the card`,
    `    // dispatcher.`,
