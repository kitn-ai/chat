...
// WORKAROUND (kit bug, filed with the Task 3.1 report): kai-view-stack's
// back() from a drill entered from a NON-DEFAULT tab root lands on the
// default root instead of the root it was entered from (repro: selectTab
// then push then back on the bare element). Until that lands, the back
// button routes through selectTab(lastRoot) - identical behavior at this
// widget's one-deep drill. Swap back to stack.back() when the fix ships.
let lastRoot = 'home';
...
$('back').addEventListener('click', () => stack.selectTab(lastRoot));
